...
func RenderTOMLForScope(c *Config, scope RenderScope) string {
	...
	if c.Language != "" {
		fmt.Fprintf(&b, "language      = %q   # ui/model language; empty = auto-detect from $LANG / $PLEXUS_LANG\n", c.Language)
	} else {
		b.WriteString("# language      = \"zh\"   # ui/model language; empty = auto-detect from $LANG / $PLEXUS_LANG\n")
	}
	if scope != RenderScopeProject {
	...
	if shouldRenderUI(c, defaults, scope) {
		b.WriteString("[ui]\n")
		fmt.Fprintf(&b, "theme = %q   # auto|dark|light; CLI colors only; PLEXUS_THEME can override per run\n", c.UITheme())
		if style := c.UIThemeStyle(); style != "" {
			fmt.Fprintf(&b, "theme_style = %q   # CLI accent palette; PLEXUS_THEME_STYLE can override per run\n", style)
		} else {
	...
	if shouldRenderNetwork(c, defaults, scope) {
		...
		if c.Network.Proxy.Password != "" {
			...
			b.WriteString("# password = \"${PLEXUS_PROXY_PASSWORD}\"   # optional; supports ${VAR} expansion\n")
		}
		b.WriteString("\n")
	}
	...
	if len(c.Plugins) == 0 {
		b.WriteString("\n[[plugins]]\n")
		b.WriteString("name    = \"filesystem\"\n")
		b.WriteString("command = \"npx\"\n")
		b.WriteString("args    = [\"-y\", \"@modelcontextprotocol/server-filesystem\", \"D:/deepseek\", \"C:/Users/19922/AppData/Roaming/plexus\"]\n")
		b.WriteString("trusted_read_only_tools = [\"list_directory\", \"read_file\", \"search_files\"]\n")
		b.WriteString("\n[[plugins]]\n")
		b.WriteString("name    = \"playwright\"\n")
		b.WriteString("command = \"npx\"\n")
		b.WriteString("args    = [\"-y\", \"@playwright/mcp\"]\n")
	} else {

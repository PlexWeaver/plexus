...
const (
	...
	credentialsKeyringService = "plexus"
...
func credentialsStoreMode() string {
	if mode := strings.TrimSpace(os.Getenv("PLEXUS_CREDENTIALS_STORE")); mode != "" {
...
func ResolveCredentialForRoot(root, key string) CredentialResolution {
	...
	// Check .env first, so a local override takes precedence
	// over the system environment variable (useful for testing).
...
func storedCredentialValue(key string) (string, CredentialSource, bool) {
	if p := UserCredentialsPath(); p != "" {
		if value, ok := envFileValue(p, key); ok {

...
func reasonixHomeDir() string {
	if dir := cleanEnvDir("PLEXUS_HOME"); dir != "" {
		return dir
	}
	...
	return filepath.Join(dir, "plexus")
}
...
func legacyXDGConfigPaths() []string {
	...
	if dir := cleanEnvDir("XDG_CONFIG_HOME"); dir != "" {
		add(filepath.Join(dir, "plexus", "config.toml"))
	}
	if home, err := os.UserHomeDir(); err == nil && home != "" {
		add(filepath.Join(home, ".config", "plexus", "config.toml"))
	}
	return paths
}

func userSupportDir() string {
	if dir := cleanEnvDir("PLEXUS_STATE_HOME"); dir != "" {
		return dir
	}
	return reasonixHomeDir()
}
...
func userCacheDir() string {
	if dir := cleanEnvDir("PLEXUS_CACHE_HOME"); dir != "" {
		return dir
	}
...
// PLEXUS_HOME/config.toml, then ~/.reasonix/config.toml on Unix-like systems,
...
// PLEXUS_HOME, then uses ~/.reasonix on macOS/Linux or %APPDATA%/reasonix on
// Windows.
...
var ConventionDirs = []string{".plexus", ".agents", ".agent", ".claude"}

...
func TestLoadForRootFiltersProjectDotEnvControlVars(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, ".config"))
	t.Setenv("AppData", filepath.Join(home, "AppData"))
	t.Setenv("PLEXUS_STATE_HOME", "")
	os.Unsetenv("PLEXUS_STATE_HOME")

	wantCred := UserCredentialsPath()
	if wantCred == "" {
		t.Skip("user credentials path unavailable")
	}
	if err := os.WriteFile(filepath.Join(project, ".env"), []byte("PLEXUS_STATE_HOME="+redirect+"\nPLUGIN_TOKEN=project-token\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(project, ".mcp.json"), []byte(`{
  "mcpServers": {
    "svc": {
      ...
      "headers": {
        ...
        "State": "${PLEXUS_STATE_HOME:-default-state}"
      }
    }
  }
}`), 0o644); err != nil {
	...
	if got := os.Getenv("PLEXUS_STATE_HOME"); got != "" {
...
func TestProjectConfigCannotOverrideCredentialStoreMode(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("PLEXUS_CREDENTIALS_STORE", "")
	os.Unsetenv("PLEXUS_CREDENTIALS_STORE")

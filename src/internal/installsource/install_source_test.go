...
func TestApplyLocalSkillFileCopiesToProject(t *testing.T) {
	...
	target := filepath.Join(project, ".plexus", "skills", "beta", "SKILL.md")
...
func TestApplyLocalSkillFileDoesNotShadowFlatCompatInstall(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	existing := filepath.Join(project, ".plexus", "skills", "beta.md")
...
func TestApplyLocalSKILLFileCopiesSiblingResources(t *testing.T) {
	...
	target := filepath.Join(project, ".plexus", "skills", "frontend-design", "SKILL.md")
	if resp.Actions[0].CanonicalPath != target {
		t.Fatalf("canonicalPath = %q, want %q", resp.Actions[0].CanonicalPath, target)
	}
	if _, err := os.Stat(filepath.Join(project, ".plexus", "skills", "frontend-design", "references", "style.md")); err != nil {
		t.Fatalf("reference file should be copied with SKILL.md source: %v", err)
	}
	if _, err := os.Stat(filepath.Join(project, ".plexus", "skills", "frontend-design", "scripts", "lint.sh")); err != nil {
...
func TestApplyLocalSkillLinkMode(t *testing.T) {
	...
	target := filepath.Join(project, ".plexus", "skills", "gamma", "SKILL.md")
...
func TestUninstallRemovesSkillByName(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	target := filepath.Join(project, ".plexus", "skills", "doomed.md")
	writeFile(t, target, "---\nname: doomed\ndescription: Doomed\n---\nbody")
...
func TestUninstallWithoutScopePrefersProjectSkill(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	projectTarget := filepath.Join(project, ".plexus", "skills", "dupe.md")
	globalTarget := filepath.Join(home, ".plexus", "skills", "dupe.md")

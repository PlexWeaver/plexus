...
func TestReleaseCacheHitGuard(t *testing.T) {
	if os.Getenv("PLEXUS_RELEASE_CACHE_GUARD") == "" {
		t.Skip("set PLEXUS_RELEASE_CACHE_GUARD=1 to run the release cache guard")
	}

	threshold := envInt("PLEXUS_CACHE_GUARD_THRESHOLD", 90)
	maxLowCases := envInt("PLEXUS_CACHE_GUARD_MAX_LOW_CASES", 1)
	...
	if len(lows) > maxLowCases {
		...
		if os.Getenv("PLEXUS_CACHE_GUARD_STRICT") != "" {
			t.Fatal(msg)
		}
	}
}

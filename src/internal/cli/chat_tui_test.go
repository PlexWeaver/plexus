...
func TestMain(m *testing.M) {
	...
	// the environment (PLEXUS_LANG/LC_ALL/LC_MESSAGES/LANG) and installs it as
...
func TestMainManagerFollowsTranscriptWithoutTopPadding(t *testing.T) {
	...
	m = m0.(chatTUI)
	m.wrappedLines = []string{"plexus", "› /mcp"}
	...
	if !strings.Contains(lines[0], "plexus") || !strings.Contains(lines[1], "/mcp") {

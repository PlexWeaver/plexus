...
model = "x"
api_key_env = "PLEXUS_TEST_KEY_UNSET"
`)
...
`)
	...
	msgs := sess.Snapshot()
	if len(msgs) != 4 {
		t.Fatalf("failed skill transcript length = %d, want 4", len(msgs))
	}
	// Strip reasoning-language prefix from user messages
	content1 := stripReasoningLang(msgs[1].Content)
	content3 := stripReasoningLang(msgs[3].Content)
	if content1 != "first skill task" || msgs[2].Content != "first skill answer" || content3 != "second skill task" {
...
`)
	...
	// Strip boot-time sections injected after the configured base prompt.
...
// stripReasoningLang removes the <reasoning-language> tag block from message content.
func stripReasoningLang(s string) string {
	const tag = "<reasoning-language>"
	if !strings.Contains(s, tag) {
		return s
	}
	// Find the closing </reasoning-language> and take everything after it
	if i := strings.Index(s, "</reasoning-language>"); i >= 0 {
		s = strings.TrimSpace(s[i+len("</reasoning-language>"):])
	}
	return s
}
...
func TestBuildMigratesLegacyConfigEndToEnd(t *testing.T) {
	...
	t.Setenv("PLEXUS_CREDENTIALS_STORE", "file")
	...
	writeFile(t, filepath.Join(home, ".plexus"), "config.json",
		`{"apiKey":"sk-e2e","lang":"zh","mcpServers":{"fs":{"command":"npx","args":["-y","server-fs"]}}}`)
	writeFile(t, filepath.Join(home, ".plexus", "sessions"), "chat-1.events.jsonl",
...
func TestBuildMigratesLegacyXDGAndProjectSessions(t *testing.T) {
	...
	t.Setenv("PLEXUS_HOME", reasonixHome)

	proj := robustTempDir(t)
	writeFile(t, proj, "reasonix.toml", "[codegraph]\nenabled = false\n")

	legacyRoot := filepath.Join(xdg, "plexus")

...
const (
	...
	debugTraceEnv      = "PLEXUS_MEMORY_COMPILER_DEBUG_TRACE"
	version            = "v5.9"
...
type Runtime struct {
	dir     string
	mu      *sync.Mutex
	learner *ProfileLearner // ArkDistill-style output compression learner
}
...
func New(dir string) *Runtime {
	...
	return &Runtime{dir: dir, mu: runtimeLockForDir(dir), learner: NewProfileLearner()}
}
...
func (t *Turn) RecordToolResults(records []ToolRecord) {
	...
	// Apply ArkDistill-style output compression to each non-error tool result.
	// Profiles adapt over time via ProfileLearner based on user corrections.
	for i, rec := range t.trace.ToolResults {
		if strings.TrimSpace(rec.Error) != "" || strings.TrimSpace(rec.Output) == "" {
			continue
		}
		outType := DetectOutputType(rec.Output)
		profile := t.rt.learner.GetProfile(outType)
		if profile == nil {
			continue
		}
		compacted, wasTruncated := CompactToolOutput(rec.Output, outType, profile)
		if wasTruncated {
			t.trace.ToolResults[i].Output = compacted
			t.trace.ToolResults[i].Truncated = true
		}
	}
}

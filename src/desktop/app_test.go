...
func isolateDesktopUserDirs(t *testing.T) string {
	...
	t.Setenv("PLEXUS_STATE_HOME", filepath.Join(home, "state"))
	t.Setenv("PLEXUS_CACHE_HOME", filepath.Join(home, "cache"))
...
func BenchmarkDesktopSettingsPayloads(b *testing.B) {
	...
	b.Setenv("HOME", home)
	b.Setenv("PLEXUS_CREDENTIALS_STORE", "file")
	b.Setenv("USERPROFILE", home)
	b.Setenv("XDG_CONFIG_HOME", xdg)
	b.Setenv("PLEXUS_STATE_HOME", filepath.Join(home, "state"))
	b.Setenv("PLEXUS_CACHE_HOME", filepath.Join(home, "cache"))
...
func TestDeleteProviderMigratesConfigAndOpenTabs(t *testing.T) {
	isolateDesktopUserDirs(t)
	setDesktopTestCredential(t, "PLEXUS_TEST_KEY", "sk-test")

	cfg := config.Default()
	cfg.DefaultModel = "prov-a/model-a2"
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", Models: []string{"model-a1", "model-a2"}, APIKeyEnv: "PLEXUS_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "PLEXUS_TEST_KEY"},
	}
...
func TestDeleteProviderRejectsRunningAffectedTab(t *testing.T) {
	...
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", APIKeyEnv: "PLEXUS_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "PLEXUS_TEST_KEY"},
	}
	if err := cfg.SaveTo(config.UserConfigPath()); err != nil {
		t.Fatalf("save config: %v", err)

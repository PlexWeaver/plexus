...
func TestMemorySuggestionsForTabUsesSelectedTab(t *testing.T) {
	...
	wantSkillPath := filepath.Join(selectedCwd, ".plexus", "skills", "selected-workflow", "SKILL.md")
	if skillPath != wantSkillPath {
		t.Fatalf("skill path = %q, want %q", skillPath, wantSkillPath)
	}
	if _, err := os.Stat(filepath.Join(activeCwd, ".plexus", "skills", "selected-workflow", "SKILL.md")); !os.IsNotExist(err) {
...
func TestMemorySuggestionsAcceptSkillCandidate(t *testing.T) {
	...
	wantSuffix := filepath.Join(".plexus", "skills", "reasonix-pr-followup", "SKILL.md")

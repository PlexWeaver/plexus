...
func TestProjectHooksSettingsUseActiveWorkspaceRootAndTrust(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(project, ".plexus", "settings.json")); err != nil {
...
func TestSaveHooksSettingsForRootUsesDisplayedProjectRoot(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(projectA, ".plexus", "settings.json")); err != nil {
		t.Fatalf("displayed project root settings missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(projectB, ".plexus", "settings.json")); err == nil {
